# Контейнер Vector
 Собственная реализация улучшенного шаблонного класса ```Vector<T>```, не уступающего по эффективности std::vector. В текущей версии контейнера используется размещающий оператор ```new``` и работа с сырой памятью.
