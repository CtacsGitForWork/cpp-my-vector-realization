...
void TestEmpty() {    
    ...
    // После Clear становится пустым
    v2.Clear();
    assert(v2.Empty());
}

void TestClear() {
    // Очистка пустого вектора
    Vector<std::string> v1;
    v1.Clear();
    assert(v1.Empty());
    assert(v1.Capacity() == 0);

    // Очистка непустого вектора
    ...
    v2.Clear();
    assert(v2.Empty());
    assert(v2.Capacity() >= 3); // Capacity не должен уменьшаться

    // Очистка после добавления элементов
    Vector<int> v3;
    v3.PushBack(1);
    v3.PushBack(2);
    v3.Clear();
...
void TestAt() {
    ...
    // Проверка исключения для недопустимых индексов
    ...
    // Изменение элемента через At
    v.At(1) = 10;
    assert(v.At(1) == 10);
}

void TestFrontBack() {
    // Проверка для вектора с одним элементом
    Vector<int> v1;
    v1.PushBack(42);
    assert(&v1.front() == &v1.back());
    assert(v1.front() == 42);

    // Проверка для вектора с несколькими элементами
    Vector<std::string> v2;
    v2.PushBack("first");
    v2.PushBack("middle");
    v2.PushBack("last");
    assert(v2.front() == "first");
    assert(v2.back() == "last");

    // Изменение элементов через front/back
    v2.front() = "new_first";
    v2.back() = "new_last";
    assert(v2[0] == "new_first");
    assert(v2[2] == "new_last");

    // Константный доступ
    const Vector<std::string>& cv = v2;
    assert(cv.front() == "new_first");
    assert(cv.back() == "new_last");
}
